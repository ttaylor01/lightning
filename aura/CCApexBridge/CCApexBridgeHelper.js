({
    callServer : function(component, helper, params) {
        ...
        var msgType = "Full";
        if (pleaseWait) {
            if ((pleaseWait.type === "Full") || (pleaseWait.type === "Toast") || (pleaseWait.type === "Spinner") || 
                (pleaseWait.type === "Event") || (pleaseWait.type === "None")) {
        ...
        if (forceRefresh) {
            ...
            console.log('[CCApexBridge.callServer] Getting ready to call Apex. ', request);
            action.setStorable({"ignoreExisting": "true"});
        } else {
            action.setStorable();
        }
        ...
        action.setCallback(this, function(response) {
            ...
            if (component.get("v.debugClient")) {
                console.log('[CCApexBridge.callServer] Back from Apex (will process response). ', {
            ...
            if (sourceComponent.isValid()) {
                ...
                if (state === "SUCCESS") {
                    ...
                    if (dataReturned.isSuccess) {
                        ...
                        if (component.get("v.debugClient")) {
                            console.log('[CCApexBridge.callServer] Back from Apex (returning to caller). ', dataReturned);
                            // debugger;
                        }
                        ...
                        console.error('[CCApexBridge.callServer] Error Message: ' + msgError);
                        if (component.get("v.debugClient")) {
                            debugger;
                        }
                        if (msgType != "None") {
                    ...
                    console.error('[CCApexBridge.callServer] Errors: ' + errors);
                    if (component.get("v.debugClient")) {
                        debugger;
                    }
    ...
    makeRecords : function(data) {
        ...
        if (record && records) {
            alert("CCApexBridge should receive the 'record' or 'records' to process, but not both");
            return;
        } else if (record) {
            records = [record];
        } else if (records) {
            if (records.length == 0) {
                alert("CCApexBridge received an empty list of 'records' to process");
                return;
            } else if (records.length == 1) {
                records = [record];
            }
        } else {
            alert("CCApexBridge should receive either 'record' or 'records' to process");
            return;
        }
    ...
    fireMessageEvent : function(component, isShow, msgType, msgText) {
        var evApp = $A.get("e.c:CCPleaseWaitEvent");
        var params = {
            isShow: isShow
        };
        if (msgType) {
