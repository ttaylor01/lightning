({
    doInit : function(component, event, helper) {
        ...
        helper.getBriefDescriptionDetailPicklistValues(component, 'Case_Brief_Description__c', 'Case_Brief_Description_Detail__c');
        helper.getSubmissionMethodPicklistValues(component, 'Case_Reason__c', 'Case_Submission_Method__c');
        helper.getPayerStatesPicklistValues(component, event, helper);
        helper.getPayerTransactionPicklistValues(component, event, helper);
	},
    ...
    doArticleSearch : function(component, event, helper) {
        event.preventDefault();

        // Build the search string based on the designated input fields
        var validSearchTerm = false;
        var searchVal = '';
        var fields = ["subject","payerName","payerId"];
        fields.forEach(function(inputField) {
            var inputCmp = component.find(inputField);
            var cmpVal = inputCmp.get('v.value');
            if(cmpVal !== undefined && (cmpVal.length >= 3)) {
                searchVal = (searchVal.length > 0)? searchVal + ' ' + cmpVal : cmpVal;
                validSearchTerm = true;
            }
        });
        console.log("validSearchTerm: " + validSearchTerm);
        console.log("searchVal: " + searchVal);
        var aList = component.find("articleListCmp");
        // do not search if search terms are < 3 characters
		if(validSearchTerm) {
            aList.set('v.searchStr', searchVal);
            aList.refresh();
        } else {
            aList.clear();
        }
    ...
    onReasonChange: function(component, event, helper) {
        ...
        if(resetDependentFields) {
            ...
            component.find('briefdescriptiondetail').set("v.options", defaultVal);
            component.find('briefdescriptiondetail').set("v.errors", null);
            component.set("v.isBriefDescriptionDetailDisable", true);
        }
    ...
    onBriefDescriptionChange: function(component, event, helper) {
        ...
        // get the map values
        var Map = component.get("v.briefDescriptionDetailFieldMap");
        ...
        if (controllerValueKey !== '--- None ---' && controllerValueKey !== '') {
            ...
            helper.fetchBriefDescriptionDetailValues(component, listOfDependentFields);
    ...
    onBriefDescriptionDetailChange: function(component, event, helper) {
        console.log("Brief Description Detail: "+event.getSource().get("v.value"));
        // Handle change in field visibility
        helper.toggleVisibility(component);
    },
    // function call on change the Dependent field
    ...
    onAtypicalProviderChange : function(component, event,helper) {
        var npiCmp = component.find('npiIdInput');
        var taxCmp = component.find('taxIdInput');
        if(component.find('atypicalprovider').get('v.checked')) {
            npiCmp.set("v.required",false);
            taxCmp.set("v.required",true);
            $A.util.removeClass(npiCmp, "slds-has-error"); // remove red border
            $A.util.addClass(npiCmp, "hide-error-message"); // hide error message
        } else {
            npiCmp.set("v.required",true);
            taxCmp.set("v.required",true);
        }
    },
    onGovernmentPayerChange : function(component, event,helper) {
